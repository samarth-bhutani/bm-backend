...
def parse(browser):
    for j in range(7):
        ...
        while i < len(class_blocks):
            ...
            new_class["start_time"] = convert_to_posix(standarize_timestring(new_class["start_time"]), day.day, day.month, day.year)
            new_class["end_time"] = convert_to_posix(standarize_timestring(new_class["end_time"]), day.day, day.month, day.year)
...
def output_file():
    ...
    with open('{}/gym_classes.json'.format(dir_path), 'w') as outfile:
